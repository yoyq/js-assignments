...
function getPolynom(...args) {
    return function (v) {
        return args.reduce((a, b, c, d)=> a + (b * Math.pow(v, d.length - 1 - c)), 0);
    }
}
...
function logger(func, logFunc) {
    return function () {
        let s = JSON.stringify(Array.from(arguments)).slice(1, -1);
