...
function getRectangleString(width, height) {
    ...
    for (let i = 0; i < height; i++) {
        if (i === 0) s = '┌' + '─'.repeat(width - 2) + '┐' + '\n';
        else if (i === height - 1) s += '└' + '─'.repeat(width - 2) + '┘' + '\n';
...
function encodeToRot13(str) {
    ...
    for (let i = 0; i < newStr.length; i++) {
        ...
        if (reg.test(newStr[i])) {
            let v = newStr[i].charCodeAt(0) + 13;
            let newChar = String.fromCharCode(v);
            if (newChar.match(reg)) {
                s += str[i] === newStr[i] ? newChar : newChar.toUpperCase();
            } else {
                newChar = String.fromCharCode(v - 26);
                s += str[i] === newStr[i] ? newChar : newChar.toUpperCase();
            }
        } else {
            s += str[i];
